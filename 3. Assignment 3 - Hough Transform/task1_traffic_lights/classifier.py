...
def classify_circles(rgb_image, circles):
    hsv_image = cv2.cvtColor(
        cv2.cvtColor(rgb_image, cv2.COLOR_RGB2BGR), cv2.COLOR_BGR2HSV
    )
    classifications = []
    ...
    for center_x, center_y, radius in circles:
        ...
        squared_distance = (col_idx - center_x) ** 2 + (row_idx - center_y) ** 2
        inner_circle_mask = (
            squared_distance <= (radius * 0.8) ** 2
        )  # slightly smaller to avoid border
        ...
        print(
            f"      Circle ({center_x},{center_y},r={radius}): "
        ...
        # Saturation > 50 and brightness > 80 -> ensure vivid color
            ...
            (avg_hue < 20 or avg_hue > 160)
            and avg_saturation > 50
            and avg_brightness > 80
        ):
