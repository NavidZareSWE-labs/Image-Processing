...
from visualize import (
    ...
    save_traffic_pipeline,  # noqa: E402
    save_grayscale,
    save_heatmap,
)
...
def _select_traffic_light_triplet(circles):
    ...
    for combo in combinations(cand_circles, 3):
        ...
        middle_to_bottom_dist = three_cir[2][1] - three_cir[1][1]
        if top_to_middle_dist < 15 or middle_to_bottom_dist < 15:
            continue

        spacing_ratio = min(top_to_middle_dist, middle_to_bottom_dist) / max(
            top_to_middle_dist, middle_to_bottom_dist
        )
...
def run(image_dir=None, output_dir=None):
    ...
    for fname in files:
        ...
        print(f"\n--- Processing: {fname} ---")
        print(
            f"  Config: blur_sigma={blur_sigma}, canny=({canny_lo},{canny_hi}), "
        ...
        circles = [
            ...
            if cy > rad * 0.6
            and cy < height - rad * 0.6
            and cx > rad * 0.6
            and cx < width - rad * 0.6
        ]
        ...
        if circles and len(circles[0]) == 4:
            ...
            classify_circles_list = [(cx, cy, cr) for (cx, cy, dr, cr) in circles]
        else:
        ...
        save_grayscale(
            edges.astype(float),
            os.path.join(output_dir, f"{stem}_edges.png"),
            title=f"Edge Map - {fname}",
        )
        save_heatmap(
            acc_best,
            os.path.join(output_dir, f"{stem}_accumulator.png"),
    ...
    print(
        f"{'Image':<22} {'Edge(s)':>8} {'Hough(s)':>9} {'Class(s)':>9} {'Total(s)':>9}"
    )
    print("-" * 60)
    for t in timing_table:
        print(
            f"{t['image']:<22} {t['edge_det']:>8.3f} {t['hough']:>9.3f} "
