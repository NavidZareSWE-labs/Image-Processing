...
def hough_circle_transform(
    edge_map, gradient_dir, r_min=10, r_max=80, r_step=1, vote_threshold_ratio=0.45
):
    H, W = edge_map.shape
    ...
    print(f"    Radius range: [{r_min}, {r_max}], step={r_step}, " f"bins={n_rad}")
    ...
    for rad_idx, rad in enumerate(radiuses):
        ...
        for dir_sign in (+1, -1):
            cand_center_x = (edge_x + dir_sign * rad * np.cos(edge_theta)).astype(
                np.int32
            )
            cand_center_y = (edge_y + dir_sign * rad * np.sin(edge_theta)).astype(
                np.int32
            )
            ...
            valid_center_x, valid_center_y = (
                cand_center_x[valid_centers],
                cand_center_y[valid_centers],
            )
            ...
            np.add.at(accumulator[:, :, rad_idx], (valid_center_y, valid_center_x), 1)

    # ---- Peak detection - Local-max check ----
    max_val = accumulator.max()
    threshold = max_val * vote_threshold_ratio
    print(f"    Accumulator max votes: {max_val}, " f"threshold: {threshold:.1f}")

    circles = []
    flat_idx = accumulator.argmax()
    best_coords = np.unravel_index(
        flat_idx, accumulator.shape
    ...
    for rad_idx, rad in enumerate(radiuses):
        ...
        for row_offset in (-1, 0, 1):
            for col_offset in (-1, 0, 1):
                ...
                neighbor = padded[row_start:row_end, col_start:col_end]
...
def remove_dup_circles(circles, min_dist=15):
    ...
    for cand_circle in circles:
        ...
        for k in keep:
            ...
            dist = np.sqrt((cand_x - accepted_x) ** 2 + (cand_y - accepted_y) ** 2)
