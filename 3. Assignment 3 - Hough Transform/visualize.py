...
def save_heatmap(accumulator, path, title="Hough Accumulator", xlabel="", ylabel=""):
...
def draw_circles_on_image(rgb_img, circles, labels):
    ...
    for (cx, cy, r), lbl in zip(circles, labels):
        ...
        circ = MplCircle((cx, cy), r, fill=False, edgecolor="magenta", linewidth=3)
        ax.add_patch(circ)
...
def save_traffic_pipeline(
    original, edges, accumulator_slice, overlay, path, img_name=""
):
    """Save 4-step pipeline figure for Part 1."""
    fig, axes = plt.subplots(1, 4, figsize=(22, 5))
    titles = ["Original", "Edge Map", "Hough Accumulator (best r)", "Detected Circles"]
