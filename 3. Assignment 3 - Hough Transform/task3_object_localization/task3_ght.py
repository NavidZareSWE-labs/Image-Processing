    ...
    for _ in range(5):
        for i in range(1, h-1):
            for j in range(1, w-1):
                if weak[i, j] == 1 and np.any(edge_map[i-1:i+2, j-1:j+2] == 1):
                    edge_map[i, j] = 1
    return edge_map
...
def generalized_hough_vote(edge_binary, edge_angle, rtable, num_angle_bins, image_shape):
...
def save_visualization(original, gray, edge_map, accumulator, detected_center, output_prefix):
...
def main():
    ...
    start = time.time()
    accumulator = generalized_hough_vote(edge_test, angle_test, rtable, num_bins, test_gray.shape)
    elapsed = time.time() - start
    ...
    save_visualization(test_original, test_gray, edge_test, accumulator, detected_center, "GHT_result")
