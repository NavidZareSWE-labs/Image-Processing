...
def plot_result_s0(SM, LG_zeros, LG_inserted, title="Diamond Embedding", filename="diamond_embedding.png"):
    fig, axes = plt.subplots(1, 3, figsize=(14, 5))
    fig.suptitle(title, fontsize=14, fontweight='bold')

    def show_matrix(ax, mat, t):
        ax.imshow(mat != 0, cmap='Blues', aspect='equal',
                  vmin=0, vmax=1)
        for i in range(mat.shape[0]):
            for j in range(mat.shape[1]):
                v = mat[i, j]
                ax.text(j, i, str(v), ha='center', va='center',
                        fontsize=7, color='black' if v == 0 else 'white')
        ax.set_title(t, fontsize=11)
        ax.set_xticks([])
        ax.set_yticks([])

    show_matrix(
        axes[0], SM, f"Smaller Matrix SM\n({SM.shape[0]}x{SM.shape[1]})")
    show_matrix(
        axes[1], LG_zeros, f"Larger Matrix LG (zeros)\n({LG_zeros.shape[0]}x{LG_zeros.shape[1]})")
...
def plot_metrics_bar_s1(all_metrics, img_name):
    ...
    for i, (mname, col) in enumerate(zip(method_names, colors)):
        ...
        ax.bar(x + i * width, psnr_vals, width, label=mname, color=col,
               edgecolor='black', linewidth=0.5)
        for j, v in enumerate(psnr_vals):
            ax.text(x[j] + i * width, v + 0.3, f"{v:.1f}", ha='center',
                    fontsize=7)
...
def plot_transform_s2(original, transformed, curve_r, curve_s,
                      title, subtitles=("Original", "Enhanced"),
...
def plot_gamma_curves_family_s2(r_range):
    ...
    ax.plot([0, 255], [0, 255], 'k--', linewidth=1.2,
            alpha=0.5, label="γ=1 (identity)")
    ax.fill_between(r_range, r_range, 255 * np.power(r_range/255.0, 0.5),
                    alpha=0.08, color='blue', label="Brightening region (γ<1)")
    ax.fill_between(r_range, r_range, 255 * np.power(r_range/255.0, 2.0),
                    alpha=0.08, color='red',  label="Darkening region (γ>1)")
    ax.set_xlabel("Input r", fontsize=11)
    ax.set_ylabel("Output s", fontsize=11)
    ax.set_title("Power-Law (Gamma) Family of Curves\n"
                 "γ<1 expands dark regions; γ>1 compresses them", fontsize=11)
...
def plot_piecewise_linear_stretching_s2(gray, enhanced, r_range):
    ...
    fig, axes = plt.subplots(1, 3, figsize=(14, 4))
    fig.suptitle(
        "Piecewise-Linear Contrast Stretching (low contrast.jpg)", fontsize=12)
    ...
    for i in range(len(r_breakpoints) - 1):
        axes[2].plot(r_breakpoints[i:i+2],
                     s_breakpoints[i:i+2], 'b-o', linewidth=2)
    axes[2].plot([0, 255], [0, 255], 'k--', alpha=0.4)
    colors_seg = ['#e74c3c', '#3498db', '#2ecc71']
    seg_labels = ['Seg 1: compress shadows\n[0,85]->[0,30]',
                  'Seg 2: stretch mid-tones\n[85,170]->[30,220]',
                  'Seg 3: clip highlights\n[170,255]->[220,255]']
    for i, (r0, r1, s0, s1) in enumerate(zip(r_breakpoints[:-1], r_breakpoints[1:],
                                             s_breakpoints[:-1], s_breakpoints[1:])):
        axes[2].fill_betweenx([s0, s1], [r0, r1],
                              alpha=0.15, color=colors_seg[i],
                              label=seg_labels[i])
    ...
    axes[2].set_ylim(0, 255)
    plt.tight_layout()
    plt.savefig("output/section2/piecewise_linear.png",
...
def plot_all_bitplanes_s2(bitplanes):
    ...
    for k in range(8):
        ...
        ax.set_title(f"Bit-plane {k}  ({'MSB' if k == 7 else 'LSB' if k == 0 else ''})",
                     fontsize=9)
        ax.axis('off')
    plt.tight_layout()
    plt.savefig("output/section2/bitplanes_all.png",
                dpi=130, bbox_inches='tight')
    plt.close()
    print("  Saved output/section2/bitplanes_all.png")


def plot_bitplane_reconstruction_comparison_s2(img_gray, bitplanes, recon_msb, recon_lsb, mse_msb, mse_lsb):
    ...
    plt.tight_layout()
    plt.savefig("output/section2/bitplane_reconstruction.png", dpi=130,
                bbox_inches='tight')
    plt.close()
    ...
    ax3.bar(labels, psnr_vals, color='steelblue',
            edgecolor='black', linewidth=0.5)
...
def plot_histogram_comparison_s3(name, img_gray, img_eq, hist_orig, hist_eq, cdf_orig, r_range, lut_curve):
    fig, axes = plt.subplots(2, 3, figsize=(16, 9))
    fig.suptitle(f'Global Histogram Equalization - {name}',
    ...
    # Row 1: histograms
    axes[1, 0].bar(r_range, hist_orig, width=1,
    ...
    axes[1, 1].bar(r_range, hist_eq, width=1,
    ...
    axes[1, 2].plot(r_range, cdf_eq,   label='Equalized CDF', linewidth=2,
                    linestyle='--')
...
def plot_histogram_equalization_s3(name, img_gray, gray_ghe, gray_lhe):
    ...
    fig.suptitle(f'GHE vs. LHE (15x15) - {name}',
                 fontsize=13, fontweight='bold')
    ...
    for ax, im, title, col in zip(
        ...
        ax.bar(r_range, compute_histogram(im), width=1,
...
def plot_zoomed_patch_comparison_s3(img_gray, gray_ghe, gray_lhe):
    ...
    for ax, im, t in zip(axes2,
        ...
        ax.axis('off')
    plt.tight_layout()
    plt.savefig('output/section3/lhe_moon_patch.png',
...
def plot_rgb_histogram_matching_results_s4(src_name, img_src, ref_rgb_raw, matched_rgb, channel_names, channel_cols):
    ...
    for ch, (cname, col) in enumerate(zip(channel_names, channel_cols)):
        ...
        axes[0, 3].plot(np.arange(256), lv, color=col,
    ...
    for ch, (cname, col) in enumerate(zip(channel_names, channel_cols)):
        ax = axes[1, ch]
        r = np.arange(256)
        ax.bar(r, compute_histogram(img_src[:, :, ch]),
               width=1, color=col, edgecolor='none', alpha=0.5,
               label='Source')
        ax.bar(r, compute_histogram(ref_rgb_raw[:, :, ch]),
               width=1, color='gray', edgecolor='none', alpha=0.4,
               label='Reference')
        ax.bar(r, compute_histogram(matched_rgb[:, :, ch]),
               width=1, color=col, edgecolor='none', alpha=0.9,
               label='Matched')
...
def plot_gray_histogram_matching_results_s4(ref_gray, src_name, src, matched, cdf_src, cdf_ref, lut_vals, h_src, h_ref, h_matched, r):
    fig, axes = plt.subplots(2, 4, figsize=(20, 9))
    fig.suptitle(
    ...
    axes[0, 3].plot([0, 255], [0, 255], 'k--', linewidth=1, alpha=0.4,
                    label='Identity')
    axes[0, 3].set_xlabel('Input  r')
    axes[0, 3].set_ylabel('Output  z')
    ...
    axes[1, 3].plot(r, cdf_src,                            label='CDF source',
                    linewidth=2, color='steelblue')
    axes[1, 3].plot(r, cdf_ref,                            label='CDF reference',
                    linewidth=2, color='tomato')
    axes[1, 3].plot(r, compute_norm_cdf(h_matched),            label='CDF matched',
                    linewidth=2, color='forestgreen', linestyle='--')
...
def plot_per_channel_histograms_s4(src_name, img, eq_rgb, eq_v, ch_names, ch_cols, r_range):
    ...
    for ch in range(3):
        for col_idx, (image, col_title) in enumerate([
            ...
            ax = axes2[ch, col_idx]
            ax.bar(r_range, compute_histogram(image[:, :, ch]),
                   width=1, color=ch_cols[ch], edgecolor='none', alpha=0.85)
