...
from visualize import save_image_row, save_robustness_plot
from utils import (
    convolve2d,
    load_grayscale,
    normalise_to_uint8,
...
def _dilate_binary(binary, radius):
    ...
    padded_h = np.pad(out, ((0, 0), (rad, rad)), mode="constant", constant_values=False)
    ...
    padded_v = np.pad(
        horizontal, ((rad, rad), (0, 0)), mode="constant", constant_values=False
    )
...
def evaluate_edge_detector(pred_binary, gt_binary, tolerance_px: int = 2):
    ...
    if n_pred == 0 or n_gt == 0:
        return {"precision": 0.0, "recall": 0.0, "f1": 0.0, "loc_error": float("inf")}
...
def run_section1():
    ...
    for img_name in image_names:
        ...
        print(
            f"    Sobel     -> magnitude range [{mag_sob.min():.2f}, {mag_sob.max():.2f}]  "
        ...
        print(
            f"    Prewitt   -> magnitude range [{mag_pre.min():.2f}, {mag_pre.max():.2f}]  "
        ...
        print(
            f"    Roberts   -> magnitude range [{mag_rob.min():.2f}, {mag_rob.max():.2f}]  "
        ...
        t_lap = time.time() - t0
        print(
            f"    Laplacian -> signed range [{lap_raw.min():.2f}, {lap_raw.max():.2f}]  "
        ...
        save_image_row(
            images=[gray, mag_sob_u8, mag_pre_u8, mag_rob_u8, lap_edge, lap_sharp],
            titles=[
                "Original",
                "Sobel",
                "Prewitt",
            ...
            output_path=os.path.join(OUT_S1, f"{img_name}_classical_operators.png"),
            suptitle=f"Task 1.1 - Classical Edge Detectors ({img_name})",
        )

        # Sobel Gx / Gy figure
        save_image_row(
            images=[normalise_to_uint8(Gx_sob), normalise_to_uint8(Gy_sob), mag_sob_u8],
            titles=[
                "Sobel Gx (normalised)",
                "Sobel Gy (normalised)",
                "Sobel Magnitude",
            ],
            output_path=os.path.join(OUT_S1, f"{img_name}_sobel_components.png"),
        ...
        for sigma in sigmas:
            ...
            print(
                f"    sigma={sigma:.1f}  kernel={gaussian_kernel_size(sigma)}x{gaussian_kernel_size(sigma)}  "
        ...
        save_image_row(
            images=canny_imgs,
            titles=canny_titles,
            output_path=os.path.join(OUT_S1, f"{img_name}_canny_multiscale.png"),
        ...
        save_image_row(
            ...
            output_path=os.path.join(OUT_S1, f"{img_name}_canny_pipeline_stages.png"),
        ...
        print(
            f"    Ground truth shape: {gt_binary.shape}  "
        ...
        for method_name, pred_bin in [
            ("Sobel (90th pct)", sobel_binary),
            ("Canny (sigma=1.5)", canny_binary),
        ]:
            metrics = evaluate_edge_detector(pred_bin, gt_binary, tolerance_px=2)
            print(f"\n    -- {method_name} --")
        ...
        save_robustness_plot(
            ...
            precision_list=sob_prec_list,
            recall_list=sob_rec_list,
            method_name=f"Sobel ({img_name})",
            output_path=os.path.join(OUT_S1, f"{img_name}_robustness_sobel.png"),
        )
        save_robustness_plot(
            noise_levels=noise_sigmas,
            precision_list=can_prec_list,
            recall_list=can_rec_list,
            method_name=f"Canny sigma=1.5 ({img_name})",
            output_path=os.path.join(OUT_S1, f"{img_name}_robustness_canny.png"),
        )
        ...
        cell_data = [
            [
                f"{sob_m['precision']:.4f}",
                f"{sob_m['recall']:.4f}",
                f"{sob_m['f1']:.4f}",
                f"{sob_m['loc_error']:.3f} px",
            ],
            [
                f"{canny_m['precision']:.4f}",
                f"{canny_m['recall']:.4f}",
                f"{canny_m['f1']:.4f}",
                f"{canny_m['loc_error']:.3f} px",
            ],
        ]
        print(cell_data)
