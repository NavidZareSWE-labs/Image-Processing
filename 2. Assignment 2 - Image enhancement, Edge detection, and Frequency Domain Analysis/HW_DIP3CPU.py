...
def crop_center(img, target_shape):
    ...
    if h < th or w < tw:
        raise ValueError(
            f"Crop error: image {img.shape} smaller than target {target_shape}"
        )
    start_h = (h - th) // 2
    start_w = (w - tw) // 2
...
def task3_manual(image_path, k_unsharp=0.8, gauss_sigma=1.2, lowpass_cutoff=40):
    ...
    ax9.imshow(img_mag_only, cmap="gray")
    ax9.set_title("9) Magnitude only")
    ax9.axis("off")
    ax10 = fig.add_subplot(gs[2, 1])
    ax10.imshow(img_phase_only, cmap="gray")
    ...
    bench_text = "Benchmark (sec)\n" + "\n".join(
        [
            f"N={sizes[i]}: Naive={times_naive[i]:.2f}, Manual FFT={times_manual_fft[i]:.4f}"
    ...
    for i, n in enumerate(sizes):
        print(
            f"N={n:3d} : Naive DFT = {times_naive[i]:.6f} sec, Manual FFT = {times_manual_fft[i]:.6f} sec"
        )
