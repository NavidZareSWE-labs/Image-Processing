...
def task3_gpu(image_path, k_unsharp=0.8, gauss_sigma=1.2, lowpass_cutoff=40):
    ...
    ax9 = fig.add_subplot(gs[2,0]); ax9.imshow(img_mag, cmap='gray'); ax9.set_title('9) Magnitude only'); ax9.axis('off')
    ax10 = fig.add_subplot(gs[2,1]); ax10.imshow(img_phase, cmap='gray'); ax10.set_title('10) Phase only'); ax10.axis('off')
    ax11 = fig.add_subplot(gs[2,2]); ax11.text(0.1,0.5, f'High-freq Energy: {high_energy:.2e}\nEnergy Ratio: {energy_ratio:.4f}', fontsize=10, bbox=dict(facecolor='white',alpha=0.8)); ax11.axis('off'); ax11.set_title('11) Frequency Metrics')
    ax12 = fig.add_subplot(gs[2,3]); ax12.text(0.1,0.5, f"GPU Accelerated (CuPy)\nImage: {base_name}", fontsize=10, bbox=dict(facecolor='white',alpha=0.8)); ax12.axis('off'); ax12.set_title('12) Info')
    ...
    for i, n in enumerate(sizes):
        print(f"N={n:3d} : Naive DFT = {times_naive[i]:.6f} sec, FFT (GPU) = {times_fft_gpu[i]:.6f} sec")
