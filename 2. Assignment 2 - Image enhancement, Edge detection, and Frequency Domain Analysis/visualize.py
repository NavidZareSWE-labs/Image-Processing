...
def save_image_row(images: list,
                   titles: list,
                   output_path: str,
                   suptitle: str = '',
                   cmap: str = 'gray',
                   col_width: float = 3.8,
                   row_height: float = 3.8) -> None:
...
def save_image_grid(images: list,
                    titles: list,
                    output_path: str,
                    ncols: int = 4,
                    suptitle: str = '',
...
def save_spectrum_figure(original: np.ndarray,
    ...
    title = f'Frequency Domain Analysis - {img_name}' if img_name else 'Frequency Domain Analysis'
...
def save_benchmark_plot(sizes: list,
                        naive_times: list,
                        fft_times: list,
                        output_path: str):

    fig, ax = plt.subplots(figsize=(7, 5))
    ax.plot(sizes, naive_times, 'o-', color='crimson',
            lw=2, ms=8, label='Naïve 2-D DFT  O(N⁴)')
    ax.plot(sizes, fft_times,   's-', color='steelblue',
            lw=2, ms=8, label='2-D FFT         O(N² log N)')
    ax.set_yscale('log')
    ...
    ax.set_title('Naïve DFT vs FFT - Execution Time Benchmark',
    ...
    for N, t_n, t_f in zip(sizes, naive_times, fft_times):
        ...
        ax.annotate(f'{speedup:.0f}x', xy=(N, t_f),
...
def save_noise_restoration_grid(clean: np.ndarray,
                                noisy_images: list,
    ...
    for i, (ni, lbl) in enumerate(zip(noisy_images, noisy_labels)):
        axes[0, i + 1].imshow(_prepare_display(ni),
    ...
    for i, (ri, lbl) in enumerate(zip(restored_images, restored_labels)):
        axes[1, i + 1].imshow(_prepare_display(ri),
                              cmap='gray', vmin=0, vmax=255)
...
def save_robustness_plot(noise_levels: list,
                         precision_list: list,
                         recall_list: list,
                         method_name: str,
                         output_path: str):
    ...
    ax.plot(noise_levels, precision_list, 'o-', color='royalblue',
            lw=2, ms=7, label='Precision')
    ax.plot(noise_levels, recall_list,   's--', color='tomato',
            lw=2, ms=7, label='Recall')
    ax.set_xlabel('Gaussian Noise sigma (added to original)', fontsize=11)
    ax.set_ylabel('Score', fontsize=11)
    ax.set_title(
        f'Robustness to Noise - {method_name}', fontsize=12, fontweight='bold')
    ax.legend(fontsize=10)
